...
function AutoCompleteGeneric<T>({
  ...
  useEffect(() => onInputChange?.(debouncedInput), [debouncedInput, onInputChange]);

  const defaultFilter = useCallback(
    (item: T, q: string) => getSuggestionLabel(item).toLowerCase().includes(q.toLowerCase()),
    [getSuggestionLabel]
  );
  ...
  const listStyle = useAnchoredList(inputRef as React.RefObject<HTMLElement>, [input, showSuggestions]);
  ...
  return (
    <div className="autocomplete-container">
      ...
      {showSuggestions &&
        createPortal(
          <ul className="autocomplete-list" style={{...listStyle,backgroundColor:colors.primaryBackground,borderRadius:8}}>
            {loading ? (
