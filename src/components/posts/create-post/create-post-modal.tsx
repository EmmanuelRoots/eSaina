  ...
  return (
    <Modal isOpen={open} onClose={onClose}>
      <form onSubmit={(e)=>e.preventDefault()}>
        <ModalHeader><Text variant="Headline">Créer une publication</Text></ModalHeader>
        <ModalBody>
          <Column gap={16}>
            <Row gap={16}>
              <img src={user?.pdpUrl} width={40} height={40} style={{borderRadius:'50%'}}/>
              <Text style={{alignContent:'center'}}>{user?.firstName +' '+ user?.lastName}</Text> 
            </Row>
            <Row>
              <textarea 
                required
        ...
        <ModalFooter>
          <Row style={{flex:1}}>
            <Button  style={{flex:1, padding:'1%', backgroundColor:colors.secondary}}  
              onClick={handleSubmit} >
              <Text variant="Headline" color='primaryBackground'>Publier</Text>
            </Button>
          </Row>
        </ModalFooter>
      </form>
