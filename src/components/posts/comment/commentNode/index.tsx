...
export const CommentNode = ({ node, depth }: CommentNodeProps) => {
  ...
  const [myReaction, setMyReaction] = useState<ReactionDTO | undefined>(
    reactions.find(r => r.user?.id === user?.id && r.comment.id === node.id) ??
      undefined
  )
  const pad = depth * 1.5
    ...
    <>
      <Row gap={16} style={{ marginLeft: `${pad}rem` }}>
        ...
        <Column style={{ flex: 1 }}>
          ...
          >
            <Row gap={12}>
              <button style={styles.commentAction} onClick={handleLiked}>
                <Text
                  color={liked ? 'likedText' : 'secondaryText'}
                  variant="body2"
              ...
              <button style={styles.commentAction}>
                <Text
                  ...
                  variant="body2"
                  style={{ fontWeight: 600 }}
                  onClick={() => setShowRep(prev => !prev)}
                >
                  Répondre
                </Text>
              </button>
              <Text variant="body2" color="secondaryText">
                {getTimeBetweenTwoDate(node.createdAt)}
              </Text>
            </Row>
      ...
      {showRep &&
        replies?.map(n => (
          <CommentNode key={n.id} depth={depth + 1} node={n} />
        ))}
      {showRep && (
        <Row style={{ marginLeft: `${pad + 1}rem` }}>
