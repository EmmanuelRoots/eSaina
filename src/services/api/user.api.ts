...
const logUser = async (credentials: LoginDTO) => {
  const { data } = await axiosInstance
    .post(urls.user.LOGIN, credentials)
    .catch(err => {
      throw err
    })
...
const googleLogin = async (credentials: GoogleLoginDTO) => {
  const { data } = await axiosInstance
    .post(urls.user.GOOGLE_LOGIN, credentials)
    .catch(err => {
      throw err
    })
...
const getUserByToken = async () => {
  const { data } = await axiosInstance
    .get(urls.user.GET_USER_BY_TOKEN)
    .catch(err => {
      throw err
    })
...
const logOut = async () => {
  ...
  const res = await axiosInstance
    .post(urls.user.LOGOUT, { refreshToken })
    .catch(err => {
      throw err
    })
...
const searchUser = async ({
  ...
  const { data } = await axiosInstance
    .get(urls.user.SEARCH_USER, { params: { searchTerm, page, limit } })
    .catch(err => {
      throw err
    })
