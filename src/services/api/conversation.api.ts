...
const getAllConversation = async ({
  ...
  const { data } = await axiosInstance
    .get(urls.conversation.GET_ALL, { params: { page, limit } })
    .catch(err => {
      throw err
    })
...
const createConversation = async (conversation: Partial<ConversationDTO>) => {
  const { data } = await axiosInstance
    .post(urls.conversation.CREATE, { conversation })
    .catch(err => {
      throw err
    })
...
const sendMessage = async (message: MessageDTO) => {
  const { data } = await axiosInstance
    .post(urls.conversation.SEND_MESSAGE, { ...message })
    .catch(err => {
      throw err
    })
...
const getAllMessage = async ({
  ...
  const { data } = await axiosInstance
    .get(urls.conversation.GET_ALL_MESSAGE, {
